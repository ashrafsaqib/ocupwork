...
// Register custom event for activity.addHistory
if (file_exists(DIR_SYSTEM . 'library/event/order_post.php')) {
	require_once(DIR_SYSTEM . 'library/event/order_post.php');
	if (class_exists('EventOrderPost')) {
		$event = new EventOrderPost();
		if (isset($registry) && $registry->has('event')) {
			$event_manager = $registry->get('event');
			$event_manager->register('activity.addHistory', [$event, 'addHistory']);
		}
	}
}
